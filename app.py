...
# --- 3. CONFIG & STYLE ---
st.set_page_config(page_title="CHERRY v14.8.5", layout="wide", page_icon="🍒")
...
st.markdown("""
    ...
    .final-amount-popup { font-size: 40px; font-weight: bold; color: #e44d26; text-align: center; padding: 10px;
    border-radius: 10px; background-color: #fff3f0; border: 2px solid #e44d26; }
    
    div.stButton > button { background-color: #d3d3d3 !important;
    color: #000000 !important; border-radius: 8px !important; font-weight: bold !important; border: 1px solid #808080 !important; }
    
    .data-row { font-family: 'Courier New', monospace; background-color: #262626; padding: 12px; border-radius: 8px;
    margin-bottom: 5px; border-left: 5px solid #3498db; display: block; white-space: pre; }
...
# --- 4. MASTER LISTS & SYNC (v14.7.8 Logic) ---
            ...
            if st.session_state.selected_cust_id is None:
                ...
                if ph:
                    ...
                    if len(clean_ph) == 10:
                        ...
                        if res.data: st.session_state.selected_cust_id, st.session_state.cust_name = res.data[0]['id'], res.data[0]['name']
        ...
        if st.text_input("Κωδικός MANAGER", type="password") == "999":
            ...
            if res_s.data:
                ...
                with t3:
                    ...
                    else: st.warning("Δεν υπάρχουν δεδομένα για την επιλεγμένη περίοδο.")

    # --- ΑΠΟΘΗΚΗ (v14.7.8 Full Logic) ---
                ...
                if st.form_submit_button("💾 ΑΠΟΘΗΚΕΥΣΗ ΠΡΟΪΟΝΤΟΣ"):
                    if f_design:
                        ...
                        try:
                            ...
                            st.success("Το προϊόν καταχωρήθηκε με Barcode: {}".format(sku))
                            time.sleep(1); st.rerun()
                        except Exception as e: st.error("Σφάλμα: {}".format(e))
