...
if st.session_state.is_logged_out:
    ...
    if current_view == "🛒 ΤΑΜΕΙΟ":
        ...
        if res_s.data:
            ...
            # Υπολογισμός ΠΡΑΞΗΣ: Αυξουσα αρίθμηση μοναδικών συναλλαγών ανά ημέρα
            df = df.sort_values(['ΗΜΕΡΟΜΗΝΙΑ', 's_date_dt'])
            df['ΠΡΑΞΗ'] = df.groupby('ΗΜΕΡΟΜΗΝΙΑ')['s_date'].transform(lambda x: pd.Series(x.unique()).rank(method='min').map(dict(zip(x.unique(), range(1, len(x.unique())+1))))).astype(int)
            ...
            with t1:
                ...
                if not tdf.empty:
                    ...
                    st.dataframe(tdf[['ΠΡΑΞΗ', 's_date', 'item_name', 'final_item_price', 'method', 'ΠΕΛΑΤΗΣ']].sort_values('s_date', ascending=False), use_container_width=True
