...
def print_label_popup(bc, name, price):
    ...
    qty = st.number_input("Πλήθος", min_value=1, max_value=50, value=1)
...
if not st.session_state.logged_in:
    ...
    if current_view == "🛒 ΤΑΜΕΙΟ":
        ...
        with t_new:
            # Χρήση st.form για να μην κάνει rerun με το Enter στα πεδία κειμένου
            with st.form("inventory_form", clear_on_submit=True):
                ...
                # Το label παραμένει, αλλά δεν στέλνει δεδομένα μέχρι το submit
                ...
                if st.form_submit_button("💾 ΑΠΟΘΗΚΕΥΣΗ"):
                    if not f_design:
                        st.error("⚠️ Πρέπει να συμπληρώσετε το Σχέδιο/Κωδικό!")
                    elif f_price <= 0:
                        ...
                        try:
                            ...
                            st.success(f"Επιτυχής Καταχώρηση: {sku}")
        ...
        with t_set:
            ...
            for v in sorted(st.session_state.master_lists.get(cat, [])):
                ...
                col1.write(v)
                if col2.button("🗑️", key=f"del_lst_{v}"):
        ...
        if res.data:
            for r in res.data:
                ...
                with c1: st.markdown(f"<div class='data-row'>👤 {r['name']} | 📞 {r['phone']}</div>", unsafe_allow_html=True)
                if c2.button("⭐", key=f"p_{r['id']}"): show_customer_history(r['id'], r['name'])
                if c3.button("❌", key=f"cd_{r['id']}"): supabase.table("customers").delete().eq("id", r['id']).execute(); st.rerun()

    elif current_view == "⚙️ SYSTEM" and supabase:
        if st.text_input("System Code", type="password") == "999":
            target = st.selectbox("Clear Table", ["---", "Sales", "Customers", "Inventory"])
            if target != "---" and st.text_input("Type ΔΙΑΓΡΑΦΗ") == "ΔΙΑΓΡΑΦΗ":
                if st.button("ΕΚΤΕΛΕΣΗ"): supabase.table(target.lower()).delete().neq("id", -1).execute(); st.rerun()
