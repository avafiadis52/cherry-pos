...
if st.session_state.is_logged_out:
    ...
    with st.sidebar:
        ...
        if HAS_MIC:
            ...
            if text:
                ...
                if nums:
                    found_price = float(nums[0])
                    price_to_add = -found_price if st.session_state.return_mode else found_price
                    st.session_state.cart.append({'bc': 'VOICE', 'name': text.upper(), 'price': price_to_add})
    ...
    if current_view == "🛒 ΤΑΜΕΙΟ":
        ...
        if res_s.data:
            ...
            with t1:
                tdf = df[df['ΗΜΕΡΟΜΗΝΙΑ'] == get_athens_now().date()].copy()
                if not tdf.empty:
                    st.metric("ΤΖΙΡΟΣ ΗΜΕΡΑΣ", f"{tdf['final_item_price'].sum():.2f}€")
            ...
            with t2:
                cs, ce = st.columns(2); sd, ed = cs.date_input("Από", date.today()-timedelta(days=7)), ce.date_input("Έως", date.today())
                # Φιλτράρισμα βάσει ημερομηνιών
                ...
                if not pdf.empty:
                    ...
                    for day in sorted(pdf['ΗΜΕΡΟΜΗΝΙΑ'].unique(), reverse=True):
                        ...
                        m_v = day_df[day_df['method'] == 'Μετρητά']['final_item_price'].sum()
                        k_v = day_df[day_df['method'] == 'Κάρτα']['final_item_price'].sum()
                        
                        c_a, c_b = st.columns(2)
                        c_a.markdown(f"<div class='report-stat' style='padding:8px; margin-bottom:5px;'>💵 Μετρητά: <b>{m_v:.2f}€</b></div>", unsafe_allow_html=True)
                        c_b.markdown(f"<div class='report-stat' style='padding:8px; margin-bottom:5px;'>💳 Κάρτα: <b>{k_v:.2f}€</b></div>", unsafe_allow_html=True)
                        
                        st.dataframe(day_df[['s_date', 'item_name', 'final_item_price', 'method']], use_container_width=True, hide_index=True)
