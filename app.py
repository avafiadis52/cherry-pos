...
def payment_popup():
    ...
    if opt == "ΝΑΙ":
        ...
        if inp:
            try:
                if "%" in inp:
                    perc = float(inp.replace("%", ""))
                    disc = round((perc / 100) * total, 2)
                else:
...
def finalize(disc_val, method):
    sub_total = sum(i['price'] for i in st.session_state.cart)
    ratio = disc_val / sub_total if sub_total > 0 else 0
    ts = get_athens_now().strftime("%Y-%m-%d %H:%M:%S")
    c_id = st.session_state.selected_cust_id if st.session_state.selected_cust_id != 0 else None
    
    try:
        for i in st.session_state.cart:
            item_disc = round(i['price'] * ratio, 2)
            final_item = round(i['price'] - item_disc, 2)
            data = {
                "barcode": str(i['bc']), "item_name": str(i['name']), 
                "unit_price": float(i['price']), "discount": float(item_disc),
                "final_item_price": float(final_item), "method": str(method),
...
def display_report(sales_df):
    if sales_df.empty:
        st.info("Δεν υπάρχουν πωλήσεις για αυτή την περίοδο.")
        return
    ...
    unique_trans = df.groupby(['day_str', 's_date']).agg({
    ...
    for day, day_data in df.groupby('day_str', sort=True):
        ...
        st.dataframe(
            day_data[['ΠΡΑΞΗ', 's_date', 'item_name', 'unit_price', 'discount', 'final_item_price', 'method', 'ΠΕΛΑΤΗΣ']]
            .sort_values('ΠΡΑΞΗ', ascending=True),
...
if view == "🛒 ΤΑΜΕΙΟ":
    ...
    with cl:
        if st.session_state.selected_cust_id is None:
            ...
            bc = st.text_input("Σάρωση Barcode", key=f"bc_{st.session_state.bc_key}", help="Πληκτρολογήστε 999 για ελεύθερο είδος")
            if bc:
            ...
            if st.session_state.cart:
                if st.button("💰 ΟΛΟΚΛΗΡΩΣΗ / ΠΛΗΡΩΜΗ", use_container_width=True): payment_popup()
            if st.button("🗑️ ΑΚΥΡΩΣΗ ΚΑΛΑΘΙΟΥ", use_container_width=True): reset_app()
    ...
    if res_all.data:
        ...
        with t1:
            display_report(full_df[full_df['s_date_dt'].dt.date == get_athens_now().date()])
        with t2:
            c1, c2 = st.columns(2)
            d_s = c1.date_input("Από:", get_athens_now().date() - timedelta(days=7))
            d_e = c2.date_input("Έως:", get_athens_now().date())
            display_report(full_df[(full_df['s_date_dt'].dt.date >= d_s) & (full_df['s_date_dt'].dt.date <= d_e)])

elif view == "📦 ΑΠΟΘΗΚΗ":
    st.subheader("📦 Διαχείριση Αποθέματος")
    with st.form("inv_form", clear_on_submit=True):
        c1, c2, c3, c4 = st.columns(4)
        b = c1.text_input("Barcode")
        n = c2.text_input("Όνομα Είδους")
        p = c3.number_input("Τιμή Πώλησης (€)", step=0.1)
        s = c4.number_input("Απόθεμα (Stock)", step=1)
        if st.form_submit_button("ΑΠΟΘΗΚΕΥΣΗ ΕΙΔΟΥΣ"):
            if b and n:
    ...
    if res.data:
        for row in res.data:
            st.markdown(f"<div class='data-row'>{row['barcode']} | <b>{row['name']}</b> | {row['price']}€ | Stock: {row['stock']}</div>", unsafe_allow_html=True)
            if st.button("Διαγραφή", key=f"inv_{row['barcode']}"):
                supabase.table("inventory").delete().eq("barcode", row['barcode']).execute()
                st.rerun()

elif view == "👥 ΠΕΛΑΤΕΣ":
    st.subheader("👥 Λίστα Πελατών")
    with st.form("c_form", clear_on_submit=True):
        cn = st.text_input("Ονοματεπώνυμο")
        cp = st.text_input("Τηλέφωνο")
        if st.form_submit_button("ΠΡΟΣΘΗΚΗ ΠΕΛΑΤΗ"):
            if cn and cp:
                supabase.table("customers").insert({"name": cn, "phone": cp}).execute()
                st.success("Προστέθηκε!"); time.sleep(0.5); st.rerun()
    
    res = supabase.table("customers").select("*").execute()
    if res.data:
        for row in res.data:
            st.markdown(f"<div class='data-row'>👤 {row['name']} | 📞 {row['phone']}</div>", unsafe_allow_html=True)
            if st.button("Διαγραφή", key=f"c_{row['id']}"):
