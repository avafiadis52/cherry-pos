...
if st.session_state.is_logged_out:
    st.markdown("<h1 style='text-align: center; color: #e74c3c; margin-top: 100px;'>?? ? ???????? ???????</h1>", unsafe_allow_html=True)
    if st.button("?? ????????????"):
...
def finalize(disc_val, method):
    ...
    try:
        ...
        st.success("???????Ω????!"); time.sleep(0.5); reset_app()
...
def display_report(sales_df):
    ...
    # Stats
    m_df = unique_trans[unique_trans['method'] == '???????']
    k_df = unique_trans[unique_trans['method'] == '?????']
...
if view == "?? ??????":
    ...
    with cl:
        if st.session_state.selected_cust_id is None:
            ...
            if st.session_state.cart:
                if st.button("?? ????Ω??", use_container_width=True): payment_popup()
    ...
    with st.form("inv_form", clear_on_submit=True):
        ...
        b, n = c1.text_input("Barcode"), c2.text_input("?????")
        p, s = c3.number_input("???? (€)", step=0.1), c4.number_input("Stock", step=1)
    ...
    if res.data:
        for row in res.data:
            ...
            if st.button("????????", key=f"inv_{row['barcode']}"):
