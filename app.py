...
if st.session_state.is_logged_out:
    st.markdown("<h1 style='text-align: center; color: #e74c3c; margin-top: 100px;'>ΑΠΟΣΥΝΔΕΘΗΚΑΤΕ ΕΠΙΤΥΧΩΣ</h1>", unsafe_allow_html=True)
    if st.button("ΕΠΑΝΑΣΥΝΔΕΣΗ"):
    ...
    with cl:
        if st.session_state.selected_cust_id is None:
            ...
            for idx, item in enumerate(st.session_state.cart):
                if st.button(f"🗑️ {item['name']} ({item['price']}€)", key=f"del_{idx}", use_container_width=True):
