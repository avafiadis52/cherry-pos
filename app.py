...
# --- 3. CONFIG & STYLE ---
st.set_page_config(page_title="CHERRY v14.8.3", layout="wide", page_icon="🍒")
...
st.markdown("""
    ...
    .cart-area { 
        font-family: 'Courier New', monospace; background-color: #000000; padding: 15px; border-radius: 10px; 
...
DEFAULT_LISTS = {
    ...
    "Συνθέσεις": ["100% Βαμβάκι", "100% Πολυέστερ", "70% Βαμβάκι - 30% Πολυέστερ", "98% Βαμβάκι - 2% Ελαστάνη"]
}
    ...
    if 'master_lists' not in st.session_state:
        if supabase:
            try:
                ...
                st.session_state.master_lists = res.data[0]['config_value'] if res.data else DEFAULT_LISTS.copy()
            except: st.session_state.master_lists = DEFAULT_LISTS.copy()
        else: st.session_state.master_lists = DEFAULT_LISTS.copy()
...
def generate_latin_code(text):
    char_map = {'Α':'A','Β':'B','Γ':'G','Δ':'D','Ε':'E','Ζ':'Z','Η':'H','Θ':'TH','Ι':'I','Κ':'K','Λ':'L','Μ':'M','Ν':'N','Ξ':'X','Ο':'O','Π':'P','Ρ':'R','Σ':'S','Τ':'T','Υ':'Y','Φ':'F','Χ':'CH','Ψ':'PS','Ω':'O','Ά':'A','Έ':'E','Ή':'H','Ί':'I','Ό':'O','Ύ':'Y','Ώ':'O'}
...
if not st.session_state.logged_in:
        ...
        with cl:
            if st.session_state.selected_cust_id is None:
                ...
                if bc:
                    ...
                    if res.data:
                        val = -float(res.data[0]['price']) if is_ret else float(res.data[0]['price'])
    ...
    # --- MANAGER (ΠΛΗΡΗΣ ΛΕΙΤΟΥΡΓΙΑ v14.5.6) ---
    elif view == "📊 MANAGER":
        st.title("📊 Manager")
        manager_pwd = st.text_input("Κωδικός Manager", type="password", key="mgr_pwd")
        if manager_pwd == "999":
            res = supabase.table("sales").select("*").execute()
            if res.data:
                df = pd.DataFrame(res.data)
                df['s_date'] = pd.to_datetime(df['s_date'])
                df['date_only'] = df['s_date'].dt.date
                
                t1, t2, t3 = st.tabs(["📅 ΣΗΜΕΡΑ", "📆 ΠΕΡΙΟΔΟΣ", "📈 INSIGHTS"])
                
                with t1:
                    today = st.session_state.manual_ts.date()
                    tdf = df[df['date_only'] == today]
                    c1, c2, c3 = st.columns(3)
                    c1.metric("Σύνολο", f"{tdf['final_item_price'].sum():.2f}€")
                    ...
                    st.dataframe(tdf[['s_date', 'item_name', 'final_item_price', 'method']].sort_values('s_date', ascending=False), use_container_width=True)

                with t2:
                    col_a, col_b = st.columns(2)
                    d1 = col_a.date_input("Από", value=date.today()-timedelta(days=7))
                    d2 = col_b.date_input("Έως", value=date.today())
                    pdf = df[(df['date_only'] >= d1) & (df['date_only'] <= d2)]
                    st.metric("Σύνολο Περιόδου", f"{pdf['final_item_price'].sum():.2f}€")
                    st.dataframe(pdf[['s_date', 'item_name', 'final_item_price', 'method']], use_container_width=True)

                with t3:
                    daily = df.groupby('date_only')['final_item_price'].sum().reset_index()
                    st.plotly_chart(px.bar(daily, x='date_only', y='final_item_price', title="Τζίρος ανά Ημέρα"), use_container_width=True)
    ...
    # --- ΑΠΟΘΗΚΗ (v14.7.8) ---
        ...
        with t_new:
            with st.form(f"inv_form_{st.session_state.form_reset_key}"):
                c1, c2, c3 = st.columns(3)
                ...
                if st.form_submit_button("ΑΠΟΘΗΚΕΥΣΗ"):
                    ...
                    st.success(f"Καταχωρήθηκε: {sku}"); st.session_state.form_reset_key += 1; time.sleep(1); st.rerun()

        with t_set:
            cat = st.selectbox("Επιλέξτε Λίστα", list(st.session_state.master_lists.keys()))
            new_v = st.text_input("Προσθήκη νέου")
            if st.button("OK"): st.session_state.master_lists[cat].append(new_v); save_master_lists(); st.rerun()
    ...
    # --- ΠΕΛΑΤΕΣ ---
        ...
        if res_c.data:
            ...
            for r in sorted(res_c.data, key=lambda x: x['name']):
                ...
                if c3.button("❌", key=f"d_cust_{r['id']}"): supabase.table("customers").delete().eq("id", r['id']).execute(); st.rerun()

    # --- SYSTEM ---
    elif view == "⚙️ SYSTEM":
        if st.text_input("System Password", type="password") == "999":
            if st.button("Διαγραφή όλων των πωλήσεων"): supabase.table("sales").delete().neq("id",-1).execute(); st.rerun()
