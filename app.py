...
if st.session_state.is_logged_out:
    ...
    if view == "🛒 ΤΑΜΕΙΟ":
        ...
        if res.data:
            ...
            with t2:
                c1, c2 = st.columns(2)
                d_f = c1.date_input("Από", get_athens_now().date())
                d_t = c2.date_input("Έως", get_athens_now().date())
                p_df = all_df[(all_df['date_only'] >= d_f) & (all_df['date_only'] <= d_t)].copy()
                if not p_df.empty:
                    gm = p_df[p_df['method'] == 'Μετρητά']['final_item_price'].sum()
                    gk = p_df[p_df['method'] == 'Κάρτα']['final_item_price'].sum()
                    ...
                    gc3.markdown(f"<div class='grand-stat'><p class='stat-label'>ΣΥΝΟΛΟ
