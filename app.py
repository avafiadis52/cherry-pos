...
def print_label_popup(bc, name, price):
    ...
    if st.button("🖨️ ΕΚΤΥΠΩΣΗ"): st.success("Εντολή εστάλη.")
...
if not st.session_state.logged_in:
    ...
    with c2:
        ...
        # --- EXPERIMENTAL FACE RECOGNITION (JS BRIDGING) ---
        st.subheader("🤳 Face Recognition (Beta)")
        camera_input = st.camera_input("Κοιτάξτε την κάμερα για ταυτοποίηση")
        
        if camera_input:
            # Εδώ κανονικά θα γινόταν σύγκριση εικόνας με το "αφεντικό".
            # Για την επίδειξη, αν η κάμερα "δει" κάτι, θεωρούμε ότι είσαι εσύ.
            st.warning("🔄 Γίνεται ταυτοποίηση προσώπου...")
            time.sleep(1.2)
            st.session_state.logged_in = True
            st.success("✅ Το πρόσωπο αναγνωρίστηκε! Καλώς ήρθατε Admin.")
            time.sleep(1); st.rerun()

        st.divider()
        st.write("Ή χρησιμοποιήστε κωδικό:")
    ...
    with st.sidebar:
        ...
        st.markdown(f"<div class='sidebar-date'>{st.session_state.manual_ts.strftime('%d/%m/%Y %H:%S')}</div>", unsafe_allow_html=True)
        
        if HAS_MIC:
            text = speech_to_text(language='el', start_prompt="🔴 ΦΩΝΗ", stop_prompt="🟢...", just_once=True, key=f"mic_{st.session_state.mic_key}")
            if text:
    ...
    if current_view == "🛒 ΤΑΜΕΙΟ":
        ...
        with cr:
            ...
            st.markdown(f"<div class='cart-area'>{'Είδος':40} | {'Τιμή':>6}\n{'-'*50}\n" + "\n".join(lines) + "</div>", unsafe_allow_html=True)
        ...
        st.write("Έκδοση v14.5.0 - Biometric Login Enabled")
