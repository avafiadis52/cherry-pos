...
# --- 2. CONFIG & STYLE ---
st.set_page_config(page_title="CHERRY v13.9.3", layout="wide")
st.markdown("""
    <style>
...
def manual_item_popup():
    m_name = st.text_input("Όνομα Είδους", key="m_name_v3")
    m_price = st.number_input("Τιμή (€)", min_value=0.0, format="%.2f", step=0.1, key="m_price_v3")
...
def payment_popup():
    ...
    opt = st.radio("Έκπτωση;", ["ΟΧΙ", "ΝΑΙ"], horizontal=True, key="pay_opt_v3")
    disc = 0.0
    
    if opt == "ΝΑΙ":
        inp = st.text_input("Ποσό ή % (π.χ. 10%):", key="p_disc_val_v3")
        if inp:
            try:
...
def display_report(df):
    ...
    cols = st.columns(5)
    # Προσθήκη πλήθους πράξεων δίπλα στο λεκτικό
    ...
    with cl:
        if st.session_state.selected_cust_id is None:
            ...
            if st.session_state.cart:
                if st.button("💰 ΠΛΗΡΩΜΗ", use_container_width=True, key="pay_v3"):
                    payment_popup()
