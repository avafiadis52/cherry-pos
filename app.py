...
if not st.session_state.logged_in:
    ...
    with st.sidebar:
        ...
        st.subheader("🎙️ Φωνή")
        if HAS_MIC:
            text = speech_to_text(language='el', start_prompt="🔴 ΜΙΛΑ", stop_prompt="🟢 ΕΠΕΞΕΡΓΑΣΙΑ", just_once=True, key="voice_{}".format(st.session_state.mic_key))
            if text:
    ...
    if view in ["🛒 ΤΑΜΕΙΟ", "🔄 ΕΠΙΣΤΡΟΦΗ"]:
        ...
        if st.text_input("Κωδικός", type="password") == "999":
            ...
            if res_s.data:
                ...
                st.metric("Συνολικός Τζίρος", "{:.2f}€".format(df['final_item_price'].sum()))
        ...
        with tab_new:
            with st.form("inventory_form", clear_on_submit=True):
                ...
                if st.form_submit_button("💾 ΑΠΟΘΗΚΕΥΣΗ"):
                    ...
                    supabase.table("inventory").upsert({"barcode": sku, "name": full_name, "price": f_price, "stock": f_stock}).execute()
                    st.success("Καταχωρήθηκε!")
        with tab_settings:
            cat = st.selectbox("Λίστα", list(st.session_state.master_lists.keys()))
            new_val = st.text_input("Προσθήκη")
            if st.button("ΟΚ"):
                if new_val not in st.session_state.master_lists[cat]:
                    st.session_state.master_lists[cat].append(new_val); st.rerun()
            st.write("Τρέχουσες τιμές (Αλφαβητικά):", ", ".join(sorted(st.session_state.master_lists[cat])))
        with tab_list:
        ...
        if res.data:
            ...
            for _, r in df.iterrows():
                ...
                col1.markdown("<div class='data-row'>👤 {} | 📱 {}</div>".format(r['name'], r['phone']), unsafe_allow_html=True)
                if col2.button("👁️", key=r['id']): show_customer_history(r['id'], r['name'])
