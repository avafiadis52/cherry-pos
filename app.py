...
# --- 3. CONFIG & STYLE (Version v14.3.02 - Fixed Warehouse Only) ---
st.set_page_config(page_title="CHERRY v14.3.02", layout="wide", page_icon="🍒")
...
def edit_customer_popup(customer):
    ...
    if st.button("Αποθήκευση Αλλαγών", use_container_width=True):
        try:
            ...
            st.success("Οι αλλαγές αποθηκεύτηκαν!"); time.sleep(1); st.rerun()
        except Exception as e: st.error(f"Σφάλμα: {e}")
        ...
        if HAS_MIC:
            ...
            if text:
                ...
                if found_price:
                    ...
                    speak_text("Δεν κατάλαβα"); st.warning("Σφάλμα: Δεν βρέθηκε το είδος")
        st.divider()
    ...
    if view == "🛒 ΤΑΜΕΙΟ":
        ...
        with cl:
            if st.session_state.selected_cust_id is None:
                ...
                if bc:
                    ...
                    else: speak_text(f"Barcode {bc} όχι"); st.error(f"Barcode {bc} όχι!"); time.sleep(1); st.session_state.bc_key += 1; st.rerun()
        ...
        if res_s.data:
            ...
            with t1:
                ...
                if not tdf.empty:
                    ...
                    cc1.markdown(f"""<div class='report-stat'>💵 Μετρητά<div class='stat-val'>{m_t['final_item_price'].sum():.2f}€</div><div class='stat-desc'>({m_t['s_date'].nunique()} πράξεις)</div></div>""", unsafe_allow_html=True)
                    cc2.markdown(f"""<div class='report-stat'>💳 Κάρτα<div class='stat-val'>{c_t['final_item_price'].sum():.2f}€</div><div class='stat-desc'>({c_t['s_date'].nunique()} πράξεις)</div></div>""", unsafe_allow_html=True)
                    cc3.markdown(f"""<div class='report-stat'>📉 Εκπτώσεις<div class='stat-val' style='color:#e74c3c;'>{tdf['discount'].sum():.2f}€</div><div class='stat-desc'>Σύνολο ημέρας</div></div>""", unsafe_allow_html=True)
                    st.dataframe(tdf[['s_date', 'item_name', 'unit_price', 'discount', 'final_item_price', 'method', 'ΠΕΛΑΤΗΣ']].sort_values('s_date', ascending=False), use_container_width=True, hide_index=True)
            with t2:
                ...
                if not pdf.empty:
                    ...
                    for day in sorted(pdf['ΗΜΕΡΟΜΗΝΙΑ'].unique(), reverse=True):
                        ...
                        day_df = pdf[pdf['ΗΜΕΡΟΜΗΝΙΑ'] == day]
                        st.dataframe(day_df[['s_date', 'item_name', 'final_item_price', 'method', 'ΠΕΛΑΤΗΣ']], use_container_width=True, hide_index=True)
        ...
        with st.form("inv_f", clear_on_submit=True):
            ...
            if st.form_submit_button("Αποθήκευση"):
                if b and n:
                    try:
                        supabase.table("inventory").upsert({"barcode": str(b).strip(), "name": str(n).upper().strip(), "price": float(p), "stock": int(s)}).execute()
        ...
        if res.data:
            ...
            c1, c2, c3 = st.columns([2, 2, 1])
            s_n, s_p = c1.text_input("🔍 Όνομα", key="cust_s_n"), c2.text_input("📞 Τηλέφωνο", key="cust_s_p")
            f_df = df_cust.copy()
            if s_n: f_df = f_df[f_df['name'].str.contains(s_n.upper(), na=False)]
            if s_p: f_df = f_df[f_df['phone'].str.contains(s_p, na=False)]
            for _, r in f_df.sort_values('name').iterrows():
